...
document.addEventListener('DOMContentLoaded', function (DOMEvent) {
	...
	function update() {
		...
		next = swap;
		genCount++;
		genCountDisplay.innerHTML = `${genCount} : Generations`;
	}
	...
	document.addEventListener('mousedown', (event) => {
		...
		if (event.clientX < gridX || event.clientX > gridX + WIDTH) { return; }
		if (event.clientY < gridY || event.clientY > gridY + HEIGHT) { return; }
